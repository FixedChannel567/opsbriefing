...
export function buildEventAnalysis(event) {
  ...
  const developments = event.articles
    ...
    .sort((a, b) => sentenceScore(b.text, evidenceTerms) - sentenceScore(a.text, evidenceTerms));
  ...
  const focusTerms = (event.focusTerms ?? []).slice(0, 5);
  ...
  return {
    ...
    storyFocus: focusTerms,
    crossSourceRead: focusTerms.length
      ? `${event.articles.length} independent outlets were admitted to this event because their reporting overlaps on ${focusTerms.join(", ")}. Articles sharing only a country or regional label were excluded.`
      : `${event.articles.length} independent outlets passed the same-story comparison for this development. Articles sharing only a broad theater label were excluded.`,
