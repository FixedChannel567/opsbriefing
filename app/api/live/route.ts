...
const topics: Topic[] = [
  ...
  { id: "cyber", label: "Cyber and strategic technology", region: "Global", keywords: ["cyberattack", "semiconductor", "chip export", "ai military"], locationNames: [], coordinates: { x: 70, y: 27 }, actors: ["National governments", "Technology companies", "Cybersecurity agencies", "Critical-infrastructure operators"], watch: ["attribution", "service disruption", "export controls"], analysis: { whyItMatters: "Cyber incidents and technology controls can affect critical services, military capacity, industrial competitiveness, and supply-chain concentration.", connection: "Track technical indicators alongside government attribution, company disclosures, export-control text, and evidence of real-world disruption.", uncertainty: "Attribution is rarely immediate, and public statements may reflect incomplete technical evidence or strategic signaling." } },
];
...
export async function GET() {
  try {
    ...
    const events = await Promise.all(selectedEvents.map(async (event) => {
      const enrichedArticles = await Promise.all(event.articles.slice(0, 4).map((article) => enrichArticle(article, event.keywords)));
